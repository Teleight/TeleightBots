...
public interface BotSettings {
    ...
    ///
    /// By default, this method returns [#DEFAULT_BOT_API_URL], but can be
    ...
    ///
    /// @return `true` if exceptions are thrown silently, otherwise `false`
    boolean silentlyThrowMethodExecution();

    /// Determines whether bot extensions are enabled.
    ///
    /// @return `true` if extensions are enabled, otherwise `false`
